...
def create_pipeline_random_forestforest(use_hashing=False, stop_words=None):
    tokenizer = Tokenizer(inputCol="Text", outputCol="words")
    remover = StopWordsRemover(inputCol="words", outputCol="filtered_words", stopWords=stop_words)
    ...
    if use_hashing:
        ...
        stages = [tokenizer, remover, hashingTF, idf, rf]
        pipeline = Pipeline(stages=stages)
        return pipeline, hashingTF, rf
    else:
        vectorizer = CountVectorizer(inputCol="filtered_words", outputCol="features")
        rf = RandomForestClassifier(featuresCol="features", labelCol="Label", numTrees=20, maxDepth=5)
        stages = [tokenizer, remover, vectorizer, rf]
        pipeline = Pipeline(stages=stages)
        return pipeline, vectorizer, rf
...
def main(name="CountVectorizer_Model"):
    ...
    if name == "CountVectorizer_Model":
        ...
        pipeline2, vectormethod2, rf = create_pipeline_random_forestforest(use_hashing=False, stop_words=stop_words)
    elif name == "HashingTF_IDF_Model":
        pipeline, vectormethod, lr = create_pipeline(use_hashing=True, stop_words=stop_words)
        pipeline2, vectormethod2, rf = create_pipeline_random_forestforest(use_hashing=True, stop_words=stop_words)
    ...
    with mlflow.start_run(run_name=f"{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}") as run1:
        ...
        logging.info(f"Model {name} training completed.")
        predictions2 = model2.transform(validate_data)
        accuracy2, precision2, recall2, f12 = evaluator(predictions2)
        mlflow.log_metric("accuracy_rf", accuracy2)
        mlflow.log_metric("precision_rf", precision2)
        mlflow.log_metric("recall_rf", recall2)
        mlflow.log_metric("f1_rf", f12)
        mlflow.spark.log_model(model2, "model_rf")
