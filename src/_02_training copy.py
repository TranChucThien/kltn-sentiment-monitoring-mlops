...
from pyspark.ml.feature import Tokenizer, StopWordsRemover, CountVectorizer, HashingTF, IDF, Word2Vec
...
def create_pipeline(vectorizer_type="count", stop_words=None):
    """
    vectorizer_type: "count", "hashing", or "word2vec"
    """
...
def main_test(name="CountVectorizer_Model"):
    # ##################################################################################################
    # Configure logging within this process
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    logging.info(f"Starting Text Classification Pipeline for {name}...")
    ...
    pipeline, vectorizer, lr_model = create_pipeline(vectorizer_type="word2vec", stop_words=stop_words)
...
if __name__ == "__main__":
    ...
    # process1.join()
    # process2.join()
    main_test(name="CountVectorizer_Model")
