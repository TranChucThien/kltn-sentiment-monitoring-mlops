...
def create_pipeline_word2vec(stop_words=None):
    # Bước 1: Tokenizer
    tokenizer = Tokenizer(inputCol="Text", outputCol="words")
    ...
    # ✅ Bước 3: Word2Vec (thay cho CountVectorizer / TF-IDF)
    word2vec = Word2Vec(vectorSize=100, minCount=1, inputCol="filtered_words", outputCol="features")
...
def tune_model_word2vec(pipeline, train_data, word2vec=None, lr=None):
    evaluator = MulticlassClassificationEvaluator(labelCol="Label", predictionCol="prediction", metricName="f1")
...
def main(name="CountVectorizer_Model"):
    ...
    elif name == "Word2Vec_Model":
        pipeline, vectormethod, lr = create_pipeline_word2vec(stop_words=stop_words)
    ...
    with mlflow.start_run(run_name=f"{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}") as run1:
        if name == "CountVectorizer_Model":
            ...
            model = tune_model_word2vec(pipeline, train_data, word2vec=vectormethod, lr=lr)
        ...
        if name == "CountVectorizer_Model":
            ...
            mlflow.spark.log_model(model, "model_word2vec")
            model_uri = f"runs:/{run1.info.run_id}/model_word2vec"
            mlflow.register_model(model_uri, "Word2Vec_Model")
            test_samples(model, experiment_name, "Word2Vec_Model", mlflow)
...
if __name__ == "__main__":
    ...
    process3 = Process(target=main, args=("Word2Vec_Model",))
    process1.start()
    process2.start()
    process3.start()
