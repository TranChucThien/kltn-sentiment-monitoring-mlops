...
def create_pipeline(use_hashing=False, stop_words=None, classifier='logistic'):
    ...
    if use_hashing:
        ...
        if classifier == 'logistic':
            ...
            model = RandomForestClassifier(featuresCol="features", labelCol="Label", seed=42) # Added seed for reproducibility
        stages = [tokenizer, remover, hashingTF, idf, model]
        pipeline = Pipeline(stages=stages)
        if classifier == 'logistic':
            return pipeline, hashingTF, model
        elif classifier == 'randomforest':
            return pipeline, hashingTF, model
    else:
        ...
        if classifier == 'logistic':
            ...
            model = RandomForestClassifier(featuresCol="features", labelCol="Label", seed=42) # Added seed for reproducibility
        stages = [tokenizer, remover, vectorizer, model]
        pipeline = Pipeline(stages=stages)
        if classifier == 'logistic':
...
def tune_model(pipeline, train_data, vectormethod, lr, use_hashing=True, classifier='logistic'):
    evaluator = MulticlassClassificationEvaluator(labelCol="Label", predictionCol="prediction", metricName="f1")

    paramGrid = ParamGridBuilder()

    if use_hashing:
        paramGrid = paramGrid.addGrid(vectormethod.numFeatures, [1000, 5000, 10000])
    else:
        paramGrid = paramGrid.addGrid(vectormethod.vocabSize, [5000, 10000])

    if classifier == 'logistic':
        paramGrid = paramGrid.addGrid(lr.regParam, [0.0, 0.01, 0.1])
    elif classifier == 'randomforest':
        paramGrid = (paramGrid
                     .addGrid(lr.numTrees, [50])
                     .addGrid(lr.maxDepth, [5])
                     .addGrid(lr.minInstancesPerNode, [1]))
...
def main(name="CountVectorizer_Model"):
    ...
    if name == "CountVectorizer_Model":
        pipeline, vectormethod, lr = create_pipeline(use_hashing=False, stop_words=stop_words, classifier='logistic')
    elif name == "HashingTF_IDF_Model":
        pipeline, vectormethod, lr = create_pipeline(use_hashing=True, stop_words=stop_words, classifier='logistic')
    elif name == "RandomForest_CV_Model":
        pipeline, vectormethod, rf = create_pipeline(use_hashing=False, stop_words=stop_words, classifier='randomforest')
        lr = rf # Rename for consistency in tuning
    elif name == "RandomForest_TFIDF_Model":
        pipeline, vectormethod, rf = create_pipeline(use_hashing=True, stop_words=stop_words, classifier='randomforest')
        lr = rf # Rename for consistency in tuning
    ...
    with mlflow.start_run(run_name=f"{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}") as run1:
        if name == "CountVectorizer_Model":
            model = tune_model(pipeline, train_data, vectormethod, lr, use_hashing=False, classifier='logistic')
        elif name == "HashingTF_IDF_Model":
            model = tune_model(pipeline, train_data, vectormethod, lr, use_hashing=True, classifier='logistic')
        elif name == "RandomForest_CV_Model":
            model = tune_model(pipeline, train_data, vectormethod, lr, use_hashing=False, classifier='randomforest')
            mlflow.log_param("numTrees", model.bestModel.stages[-1].getNumTrees)
            mlflow.log_param("maxDepth", model.bestModel.stages[-1].getMaxDepth())
            mlflow.log_param("minInstancesPerNode", model.bestModel.stages[-1].getMinInstancesPerNode())
        elif name == "RandomForest_TFIDF_Model":
            model = tune_model(pipeline, train_data, vectormethod, lr, use_hashing=True, classifier='randomforest')
            mlflow.log_param("numTrees", model.bestModel.stages[-1].getNumTrees)
            mlflow.log_param("maxDepth", model.bestModel.stages[-1].getMaxDepth())
            mlflow.log_param("minInstancesPerNode", model.bestModel.stages[-1].getMinInstancesPerNode())
        ...
        elif "CountVectorizer" in name or "HashingTF_IDF" in name:
        ...
        if name == "CountVectorizer_Model":
            ...
            mlflow.spark.log_model(model, "model_rf_cv")
            model_uri = f"runs:/{run1.info.run_id}/model_rf_cv"
            mlflow.register_model(model_uri, "RandomForest_CV_Model")
            test_samples(model, experiment_name, "RandomForest_CV_Model", mlflow)
        elif name == "RandomForest_TFIDF_Model":
            mlflow.spark.log_model(model, "model_rf_tfidf")
            model_uri = f"runs:/{run1.info.run_id}/model_rf_tfidf"
            mlflow.register_model(model_uri, "RandomForest_TFIDF_Model")
            test_samples(model, experiment_name, "RandomForest_TFIDF_Model", mlflow)
...
if __name__ == "__main__":
    ...
    # process3 = Process(target=main, args=("RandomForest_CV_Model",))
    # process4 = Process(target=main, args=("RandomForest_TFIDF_Model",))
    ...
    # process3.join()
    # process4.join()
    # main(name="CountVectorizer_Model")
    # main(name="HashingTF_IDF_Model")
    main(name="RandomForest_CV_Model")
    main(name="RandomForest_TFIDF_Model")
